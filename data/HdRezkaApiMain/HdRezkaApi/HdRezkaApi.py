...
class HdRezkaApi():
	...
	def seriesInfo(self):
		...
		for i in self.translators:
			...
			r = requests.post("https://hdrezkayh3h2p.org/ajax/get_cdn_series/", data=js, headers=self.HEADERS)
			response = r.json()
			if response['success']:
	...
	def getStream(self, season=None, episode=None, translation=None, index=0):
		def makeRequest(data):
			r = requests.post("https://hdrezkayh3h2p.org/ajax/get_cdn_series/", data=data, headers=self.HEADERS)
			r = r.json()
			if r['success']:
